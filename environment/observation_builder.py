...
class ObservationBuilder:
    ...
    def build(self, env, raw_obs, info, prev_action_idx=0):
        """Assembles the full observation vector by concatenating ego state, navigation
        features, sensor extras, nav extras, a normalised previous-action scalar, and
        lidar readings in that order."""
    ...
    def _nav_extras(self, info):
        """Extracts and normalises the four navigation scalars: command float, left and
        right lane-boundary distances, and lane-centre ratio."""
    ...
    def _to_vector(self, raw_obs):
        """Converts a raw observation (dict or array) to a flat float32 vector by
        concatenating ego_state, navigation, and lidar keys in order, or falling back
        to direct array conversion."""
    ...
    def _fallback_from_agent(self, env, info):
        """Constructs ego, nav, and lidar arrays directly from env internals when the
        raw observation is shorter than the expected 259-element structure."""
    ...
    def _read_lidar_sensor(self, env):
        """Reads 240 laser returns from the lidar sensor, clips to [0, 1], and returns
        a zero array on any failure."""
    ...
    def _sensor_extras(self, env):
        """Reads side_detector and lane_line_detector outputs and concatenates them into
        a single clipped array, returning empty if neither sensor is accessible."""
